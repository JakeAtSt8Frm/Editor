...
class TecFile {
    constructor() {
        ...
        this.caf_blobs = {};   // blob_id (1..6) -> ProtoField (large field-8006 payloads)
    }
...
// Collect the 6 large field-8006 blobs in stable appearance order. Returns
// {1: ProtoField, 2: ProtoField, ...} keyed by 1-based blob index.
function _collectCafBlobs(group1601) {
    const big = [];
    function walk(fields) {
        for (const f of fields || []) {
            if (f.field_id === 8006 && f.wire_type === LENGTH_DELIMITED &&
                f.payload && f.payload.length > 100000) {
                big.push(f);
            }
            if (f.children) walk(f.children);
        }
    }
    walk(group1601.children || []);
    const out = {};
    big.forEach((bf, i) => { out[i + 1] = bf; });
    return out;
}

// Encode a parameter's *stored* value bytes as they appear inside CAF blobs.
// Mirrors _param_to_caf_bytes in tec_model.py — raw little-endian primitives,
// no display-scaling applied.
function paramToCafBytes(param) {
    if (!param) return null;
    const dt = normalizeDtype(param.data_type || '');

    if (param.byte_values && param.byte_values.length) {
        return new Uint8Array(param.byte_values);
    }

    let ivals = (param.int_values && param.int_values.length) ? [...param.int_values] : [];
    if (!ivals.length && param.int_scalar_value != null) ivals = [param.int_scalar_value | 0];
    const fvals = (param.float_values && param.float_values.length) ? param.float_values : [];

    if (dtypeIsInteger(dt)) {
        if (!ivals.length && fvals.length) {
            ivals = fvals.map(raw => {
                let sem;
                try { sem = Math.round(parseFloat(raw)); } catch (e) { sem = 0; }
                sem = clampSemanticInt(sem, dt);
                return semanticIntToStorage(sem, dt);
            });
        }
        if (ivals.length) {
            const ints = ivals.map(v => Number(v) | 0);
            const widthMap = { int8: 1, uint8: 1, int16: 2, uint16: 2, int32: 4, uint32: 4 };
            const w = widthMap[dt] ?? 4;
            const buf = new ArrayBuffer(ints.length * w);
            const dv = new DataView(buf);
            for (let i = 0; i < ints.length; i++) {
                if (w === 1) dv.setUint8(i, ints[i] & 0xFF);
                else if (w === 2) dv.setUint16(i * 2, ints[i] & 0xFFFF, true);
                else dv.setUint32(i * 4, ints[i] >>> 0, true);
            }
            // Note: int64/uint64 not encoded here (rare in CAF). Fall through to 32-bit.
            return new Uint8Array(buf);
        }
    }

    if (fvals.length) {
        const is64 = dt === 'float64';
        const w = is64 ? 8 : 4;
        const buf = new ArrayBuffer(fvals.length * w);
        const dv = new DataView(buf);
        for (let i = 0; i < fvals.length; i++) {
            if (is64) dv.setFloat64(i * 8, parseFloat(fvals[i]), true);
            else dv.setFloat32(i * 4, parseFloat(fvals[i]), true);
        }
        return new Uint8Array(buf);
    }

    if (ivals.length) {
        const buf = new ArrayBuffer(ivals.length * 4);
        const dv = new DataView(buf);
        for (let i = 0; i < ivals.length; i++) dv.setUint32(i * 4, ivals[i] >>> 0, true);
        return new Uint8Array(buf);
    }
...
class TecParser {
    ...
    parse(progressCallback = null) {
        ...
        // Collect CAF blobs (large field-8006 payloads) — used by offset-discovery tools.
        tec.caf_blobs = _collectCafBlobs(group1601);
